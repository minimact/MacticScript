...
/// Uses converted helper modules from rustscript-plugin-minimact/
...
writer MinimactTranspiler {
    ...
    pub fn visit_function_declaration(node: &FunctionDeclaration) {
        ...
        if let Some(ref mut render_body) = component.render_body {
            assign_paths_to_jsx(render_body, "", &mut self.hex_path_gen);
        }
